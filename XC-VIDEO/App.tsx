...
export const App = () => {
    ...
    // --- Persistence ---
    useEffect(() => {
        if ((window as any).aistudio) (window as any).aistudio.hasSelectedApiKey().then(hasKey => { if (!hasKey) (window as any).aistudio.openSelectKey(); });
    ...
    const handleCanvasMouseDown = (e: React.MouseEvent) => {
        ...
        if (e.button === 1 || (e.button === 0 && (e.shiftKey || isSpacePressed))) {
            setIsDraggingCanvas(true); setLastMousePos({ x: e.clientX, y: e.clientY });
            return;
        }
    ...
    useEffect(() => {
        ...
        const handleKeyDownSpace = (e: KeyboardEvent) => { if (e.code === 'Space' && (e.target as HTMLElement).tagName !== 'INPUT' && (e.target as HTMLElement).tagName !== 'TEXTAREA') { setIsSpacePressed(true); document.body.classList.add('cursor-grab-override'); } };
        const handleKeyUpSpace = (e: KeyboardEvent) => { if (e.code === 'Space') { setIsSpacePressed(false); document.body.classList.remove('cursor-grab-override'); } };
